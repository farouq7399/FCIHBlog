<?php
	class template{
	/**
	===================APD => Additional Persing Data (drop down list )
	...
	{
		#func_get_args() // Return an item from the argument list variable size
	 $bits = func_get_args();
	 $content = "";
	 foreach( $bits as $bit )
	 {
	...
	private function replaceTags( $pp = false )
	{
	 ...
	 {
	 // if the tag is an array, then we need to do more than asimple find and replace!
	 if( is_array( $data ) )
	 {
		 if( $data[0] == 'SQL' )
		 {
